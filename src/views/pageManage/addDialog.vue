...
export default {
  ...
  methods: {
    ...
    // 获取所有的目录
    openCascader () {
      $gc.post(`/category/tree`, { searchKey: '', typeList: ['catalog'], sort: false }).then(data => {
    ...
    init (nodeData, parentNode, type, categories) {
      ...
      this.$nextTick(() => {
        if (this.dataForm.type === 'bigScreen') {
          if (code) {
            $gc.get(`/${this.dataForm.type}/design/info/code/${code}`).then((resp) => {
    ...
    toDesign (form) {
      ...
      const { href: bigScreenHref } = this.$router.resolve({
        path: window.SITE_CONFIG?.designUrl || '/big-screen/design',
        query: {
          code: form.code
        }
      })
