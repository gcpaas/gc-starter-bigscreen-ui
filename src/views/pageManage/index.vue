...
import BigScreenRun from '@/views/bigScreen/components/Run/index.vue'
import Icon from '@/assets/images/pageIcon/export'
import { getPageType } from './utils/index.js'
...
export default {
  ...
  methods: {
    ...
    openCascader (node) {
      ...
      $gc.post(`/category/tree`, { searchKey: '', typeList: ['catalog'], excludeCategory, sort: false }).then(data => {
    ...
    getDataList () {
      ...
      $gc.post(`/category/tree`, { searchKey: this.searchKey, sort: this.sort }).then(data => {
    ...
    code (nodeData, node) {
      axios({
        method: 'get',
        url: `${window.SITE_CONFIG['baseUrl']}/code/generation/adminPage/${nodeData.code}`,
        headers: {
          token: $gc.getToken()
        },
    ...
    copyPege (nodeData, node) {
      $gc.post(`/${nodeData.type}/design/copy/${nodeData.code}`).then(() => {
        this.getDataList()
      })
    },
    // 新增大屏或新增页面，从空白新建
    ...
    useIt (pageTemplateId, parentNode, type) {
      ...
      const className = 'com.gccloud.starter.lowcode.page.bigscreen.dto.BigScreenPageDTO'
      $gc.post(`/${type}/design/add/template`, {
        pageTemplateId,
    ...
    addCatalog () {
      this.$refs['form'].validate(async (valid) => {
        ...
        if (this.isAdd) {
          ...
          $gc.post('/category/update', { ...this.catalogData, excludeCategory: this.catalogData.code }).then(data => {
            this.catalogVisible = false
            this.getDataList()
          }).catch(() => {
          })
...
<style lang="scss" scoped>
  ...
  .el-dropdown-menu--mini /deep/ .el-dropdown-menu__item{
      display: flex;
 ...
 /* 去除tree前面的小三角形*/
  .page-list-tree /deep/ .el-tree-node__content > .el-tree-node__expand-icon{
    width: 20px;
    opacity: 0 !important;
  }
  /*选中树节点后*/
