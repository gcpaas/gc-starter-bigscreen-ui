...
import { saveScreen } from '@/views/bigScreen/api/bigScreenApi'
import ChooseTemplateDialog from '@/views/pageManage/ChooseTemplateDialog.vue'
import _ from 'lodash'
import { stringifyObjectFunctions } from '@/views/bigScreen/utils/evalFunctions'
export default {
  name: 'PageTopSetting',
  components: {
    ChooseTemplateDialog
  ...
  methods: {
    ...
    backManagement () {
      this.$router.push({ path: window.SITE_CONFIG?.pageManagementUrl || '/pageManagement' })
    },
    // 清空
    empty () {
    ...
    preview () {
      const { href } = this.$router.resolve({
        path: window.SITE_CONFIG?.previewUrl || '/preview/bigScreen',
        query: {
          code: this.pageCode
        }
      })
